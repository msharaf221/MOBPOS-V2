...
- **تتبع الأرصدة** مع كل مورد + **سداد دفعات** من أي خزنة
- **فواتير شراء (توريد)** بتزوّد المخزون تلقائياً — كمية عادية أو أجهزة بسيريال IMEI
- تحديث **سعر التكلفة** بآخر سعر شراء (اختياري)
- **فواتير شراء آجلة** مع المتبقي على حساب المورد + طباعة نسخة داخلية
...
### IndexedDB مش فيها حد لعدد المنتجات
الحد الحقيقي هو **مساحة القرص** بس (شايفه في الإعدادات ← حجم البيانات المستخدم).
الأرقام اللي في الكود هي اللي بتحدد السعة العملية:

| البند | الحد |
|---|---|
| استعادة نسخة احتياطية | 100,000 سجل لكل مخزن، وحجم الملف ≤ 50MB |
| اسم المنتج / الكود / الباركود | 200 / 100 / 100 حرف |
| عدد المنتجات المريح | عشرات الآلاف — بشرط التقسيم، وإلا الرسم نفسه هو الاختناق |

### إيه اللي بيخلي النظام يقف مع كتالوج كبير (وإزاي اتحلّ)
- **كتابة الفروقات:** كل تعديل كان بيعيد كتابة المخزن كامل (`clear` + `put` لكل سجل)،
  يعني إضافة N منتج = N²/2 عملية كتابة. بقى الفرق بس يترسم: إضافة 1000 منتج
  نزلت من 500,500 عملية كتابة إلى 1000.
- **فهارس قاعدة البيانات (IndexedDB v5):** ترقية آمنة إضافية (Additive Migration) مع إضافة
  فهارس على `code` و `barcode` و `categoryId` للمخزون، و `imei1` و `inventoryId` للأجهزة،
  و `customerId` و `invoiceNumber` للمبيعات، و `safeId` للخزائن دون مسح أي بيانات سابقة.
- **فهارس بدل المسح:** كمية IMEI المتاحة، أسماء الفئات، بيانات العملاء — كلها
  `Map` بتتبني مرة واحدة لكل ريندر بدل `.filter()/.find()` لكل صف.
- **منسّقات متخزّنة:** `Intl.NumberFormat` بيتبني مرة لكل (لغة + عملة) بدل كل خلية.
- **تقسيم صفحات:** كل الجداول والشبكات (مخزون/مبيعات/عملاء/IMEI/موردين/حسابات
  جانبية/مستخدمين/خزينة) ليها شريط تقسيم 20/50/100/250، والاختيار بيتفتكر.
  POS بيحمّل 60 كارت و«عرض المزيد»، ولوحة المعلومات والصيانة ليها «عرض الكل».
...
│   │   ├── Purchases.tsx       # فواتير المشتريات والتوريد
...
│   │   ├── useIndexedDB.ts     # طبقة التخزين المحلي (كتابة الفروقات + دمج الكتابات)
│   │   ├── usePagination.ts    # تقسيم صفحات موحّد لكل القوائم
...
│   │   ├── barcode.ts          # توليد وطباعة الباركود (EAN-13 صالح دائمًا)
│   │   ├── format.ts           # منسّقات عملة/تاريخ متخزّنة (Intl cache)
│   │   ├── stockCounts.ts      # فهارس الكميات المتاحة (IMEI) بدل المسح المتكرر
...
### لإصدار نسخة جديدة (تلقائياً عبر CI):
1. غيّر رقم الإصدار في `package.json` + أضف بند في `CHANGELOG.md`
2. ارفع tag باسم `v{الإصدار}` (مثلاً `v1.0.2`) على `main`
3. الـ workflow (`release.yml`) يبني EXEs وينشر GitHub Release مع `latest.yml` تلقائياً ✅
4. كل الأجهزة تحدث نفسها تلقائياً عند التشغيل الجاي أو خلال 6 ساعات

### للتأكد إن التحديث هيوصل فعلاً لكل العملاء:
```bash
npm run verify:updates          # بفحص القناة بدون مصادقة — نفس طلب العميل بالظبط
npm run verify:updates -- --full   # + تحميل ملف الـ EXE والتحقق من sha512
```
- ✅ كل الفحوصات نجحت = العملاء هيلقوا التحديث تلقائياً.
- ❌ HTTP 404 = الريبو **private** (أو الـ Release ناقص) — تطبيق العميل بيدور على `latest.yml`
  بدون أي مصادقة فمش هيقدر يجيب التحديث. الحل: خلي ريبو الـ releases public،
  أو وزع التحديث بتوكن `electron-updater` (قرار أمني: التوكن هيتوزع مع كل نسخة من التطبيق).

### يدوياً (بدون CI):
1. ابنِ `npm run dist:win`
2. أنشئ GitHub Release وارفع ملفات `release/` (**يجب رفع `latest.yml` مع ملف الـ `.exe` ليعمل التحديث التلقائي**)
3. كل الأجهزة تحدث نفسها تلقائياً ✅
